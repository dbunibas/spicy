...
public class MyPopupProviderConnectionMappingTask implements PopupMenuProvider, ActionListener {

    private static Log logger = LogFactory.getLog(MyPopupProviderConnectionMappingTask.class);
    ...
    public MyPopupProviderConnectionMappingTask(Scene scene) {
        this.scene = scene;
        createPopupMenu();
    }
...
//        VMDNodeWidget vMDNodeWidget = infoCreator.createPropertyWidget(scene, bindingGroup, connectionInfo, connection, true);
        if (connectionInfo == null) {
            return;
        }
        infoCreator.createPropertyItem(itemConfidence, itemImplied, bindingGroup, connectionInfo, connection, true);
        bindingGroup.bind();
