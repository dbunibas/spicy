...
public class XQueryExpenseDBStatDB extends XQueryTest {

    protected void setUp() throws Exception {
        String mappingTaskFile = new File(this.getClass().getResource(References.expenseDBStatDBChaining).toURI()).getAbsolutePath();
        mappingTask = daoMappingTask.loadMappingTask(mappingTaskFile);
        initFixture();
    }

    protected void initFileReferences() {
        sourceInstanceXMLFile = this.getClass().getResource(References.expenseDBXMLInstance).getPath();
        expectedInstanceFile = this.getClass().getResource(References.statDBXMLExpectedInstance).getPath();
    }
