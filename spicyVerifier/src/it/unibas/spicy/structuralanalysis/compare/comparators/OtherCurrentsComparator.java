...
public class OtherCurrentsComparator implements IFeatureComparator {

    private Log logger = LogFactory.getLog(this.getClass());
    private double weight;

    public OtherCurrentsComparator() {
    }

    public OtherCurrentsComparator(Double weight) {
        this.weight = weight;
    }
    ...
    public void compareFeature(SimilarityCheck similarityCheck) {
        ...
        if (firstCurrents.length == 0 || secondCurrents.length == 0) {
            logger.warn("Circuits have different sizes. Similarity = 0");
            double similarity = 0.0;
            similarityCheck.addFeature(SpicyConstants.OTHER_CURRENTS, similarity, getWeight());
            return;
        }
        //NOTICE: output currents may have different sizes due to caching
        ...
        for (String key : currentMap1.keySet()) {
            if (key.contains(IBuildCircuitStrategy.LOAD)) {
                continue;
            }
    ...
    public String toString() {
        return "Other currents comparator - WEIGHT= " + getWeight();
    }
