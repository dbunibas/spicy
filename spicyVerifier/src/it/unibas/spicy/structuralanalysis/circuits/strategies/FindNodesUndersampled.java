...
public class FindNodesUndersampled implements IFindNodesToExclude {
    ...
    public FindNodesUndersampled() {}
    
    public FindNodesUndersampled(int minSampleSize) {
    ...
    public List<PathExpression> findAttributesToExclude(INode schemaWithSamples, SimilarityCheck similarityCheck) {
        FindUndersampledNodesVisitor visitor = new FindUndersampledNodesVisitor(minSampleSize);
    ...
    private void checkEmptyCircuits(INode schemaRoot, List<PathExpression> exclusionList) {
        ...
        if (exclusionList.contains(rootPath)) {
            throw new EmptyCircuitException("FindNodesUndersampled - Cirtuit is empty: " + schemaRoot + " - Exclusion list: " + exclusionList);
...
class FindUndersampledNodesVisitor implements INodeVisitor {
    ...
    public FindUndersampledNodesVisitor(int minSampleSize) {
        this.minSampleSize = minSampleSize;
    }
    ...
    public void visitAttributeNode(AttributeNode node) {
        ...
        toExclude = isToExclude(node);
        if (toExclude) {
            result.add(pathToNode);
        }
    }
    
    private boolean isToExclude(INode node) {
