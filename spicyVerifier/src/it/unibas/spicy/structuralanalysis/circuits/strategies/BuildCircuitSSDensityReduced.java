...
public class BuildCircuitSSDensityReduced implements IBuildCircuitStrategy {
    ...
    public BuildCircuitSSDensityReduced() {}
    
    public BuildCircuitSSDensityReduced(IFindNodesToExclude excluder, double multiplyingFactorForElements,
    ...
    public Circuit buildAndSolveCircuit(INode schema, SimilarityCheck similarityCheck, boolean excludeNodes) {
        Circuit circuit = buildCircuit(schema, similarityCheck, excludeNodes);
        if (circuit != null) {
            generateMatrix(circuit);
            solveCircuit(circuit);
            if (logger.isDebugEnabled()) logger.debug("Circuit: " + circuit);
        } else {
            if (logger.isDebugEnabled()) logger.debug("BuildCircuitSSDensityReduced.buildAndSolveCircuit = Generated a NullCircuit");
    ...
    private Circuit buildCircuit(INode schema, SimilarityCheck similarityCheck, boolean excludeNodes) {
        ...
        try {
            ...
            BuildCircuitSSDensityReducedVisitor visitor = new BuildCircuitSSDensityReducedVisitor(multiplyingFactorForElements,
...
class BuildCircuitSSDensityReducedVisitor implements INodeVisitor {
    ...
    public BuildCircuitSSDensityReducedVisitor(double multiplyingFactorForElements, double multiplyingFactorForStatisticElements, double levelResistance,
    ...
    public void visitAttributeNode(AttributeNode node) {
        ...
        CircuitNode densityNode = new CircuitNode(nodeCounter++);
        double density = featureCalculator.getNodeDensity(node);
        ...
        ICircuitElement densityResistor = new Resistor(resistorCounter++, IBuildCircuitStrategy.DENSITY, density, node, fatherNode, densityNode);
        circuit.addElement(densityResistor);
        CircuitNode elementNode = new CircuitNode(CircuitNode.GENERATOR_SOURCE - generatorCounter++);
        ICircuitElement consistencyResistor = new Resistor(resistorCounter++, IBuildCircuitStrategy.CONSISTENCY, consistency * multiplyingFactorForElements, node, densityNode, elementNode);
        ICircuitElement stressGenerator = new Generator(-generatorCounter, IBuildCircuitStrategy.STRESS, stress * 1.0, node, elementNode, groundNode);
        circuit.addElement(consistencyResistor);
        circuit.addElement(stressGenerator);
        // generator source nodes are numbered with negative numbers to simplify matrix generation
        CircuitNode elementNodeForIC = new CircuitNode(CircuitNode.GENERATOR_SOURCE - generatorCounter++);
        ICircuitElement resistorForConsistencyIC = new Resistor(resistorCounter++, IBuildCircuitStrategy.STATISTIC_CONSISTENCY, consistencyIC * multiplyingFactorForStatisticElements, node, densityNode, elementNodeForIC);
        ICircuitElement generatorForStressIC = new Generator(-generatorCounter, IBuildCircuitStrategy.STATISTIC_STRESS, stressIC * stress, node, elementNodeForIC, groundNode);
        circuit.addElement(resistorForConsistencyIC);
        circuit.addElement(generatorForStressIC);
    }
    ...
    private void addExternalResistance(Circuit circuit) {
        int numberOfResistors = BuildCircuitSSReducedWithCaching.getNumberOfResistors(circuit);
...
class MatrixGeneratorDensityReducedVisitor implements ICircuitVisitor {
    ...
    private int findMatrixDimension(Circuit circuit) {
        ...
        int numberOfGenerators = BuildCircuitSSReducedWithCaching.getNumberOfGenerators(circuit);
