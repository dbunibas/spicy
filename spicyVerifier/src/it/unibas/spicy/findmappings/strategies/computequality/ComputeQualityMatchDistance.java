...
public class ComputeQualityMatchDistance implements IComputeQuality, Comparator<AnnotatedMappingTask> {
...
//        mappingTaskSolver.solve(mappingTask);
        ...
        for (ValueCorrespondence correspondence : correspondences) {
            sumOfSquareValues += Math.pow(1.0 - correspondence.getConfidence(), 2);
        }
        double distance = Math.sqrt(sumOfSquareValues);
        annotatedMappingTask.setQualityMeasure(distance);
    }
       
    public int compare(AnnotatedMappingTask mt1, AnnotatedMappingTask mt2) {
        double firstDistance = (Double)mt1.getQualityMeasure();
        double secondDistance = (Double)mt2.getQualityMeasure();
        double difference = firstDistance - secondDistance;
        if (difference > 0.0) {
            return 1;
